...
for (mes in 1:12) {
  ...
  if (file.exists(cs2_cens_file) && file.exists(tc1_cens_file)) {
    ...
    cs2_cens_subset <- cs2_cens %>% select(NIU, DIUM) %>% distinct()
  ...
  if (file.exists(cs2_atenea_file) && file.exists(tc1_atenea_file)) {
    ...
    cs2_atenea_subset <- cs2_atenea %>% select(NIU, DIUM) %>% distinct()
...
## SAIDI Base Mensual - CENS vs ATENEA
...
kable(tabla_referencia,
      col.names = c("Mes", "SAIDI CENS", "Usuarios CENS", "SAIDI ATENEA", "Usuarios ATENEA"),
      caption = "Indicadores SAIDI Base por Empresa - Año 2024")
```
...
datos_referencia <- saidi_referencia %>%
...
ggplot(datos_referencia, aes(x = mes, y = SAIDI, color = Empresa, group = Empresa)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
...
meses_con_aumento <- sum(escenario_analizado$diferencia > 0, na.rm = TRUE)
meses_con_reduccion <- sum(escenario_analizado$diferencia < 0, na.rm = TRUE)

cat("5. **Tendencia General:** De los 12 meses analizados,", meses_con_aumento, 
